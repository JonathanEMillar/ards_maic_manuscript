...
A source of tension in our approach is the balance between the dispartiy in study designs (e.g., susceptibilty or sub-phenotype) and the requirement for sufficient data to make meaningful inferences. In an attempt to partially resolve this, we undertook MAIC on subsets of the included gene lists stratified by study focus. There were sufficient lists to make this tractable for studies focused on susceptibility to ARDS (n=28) and studies of ARDS survival or severity (n=7). For susceptibility, MAIC ranked 2096 genes (@fig-fig4). The majority of these (n=1222, 58%) were unique to susceptibility-based lists (@fig-fig4). Most were identified in blood only, with only a small fraction found solely in airways samples.  

![**MAIC on sub-groups**.](./img/Figure_4.png){#fig-fig4}
